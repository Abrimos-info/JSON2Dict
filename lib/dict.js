            ...
            value.map( v => {
                ...
                switch(itemType) {
                    ...
                    default:
                        ...
                        // console.log(fieldType, path, v);
                        updateFields(dict, itemType, path, v);
                        break;
                }
            })
            break;
...
function updateFieldProperties(field, type, value) {
    ...
    if(type == 'number') {
        ...
        if(value.length > 64) value = value.substring(0, 64); // TODO: do not truncate when string is a URI
...
function outputField(field) {
    ...
    field.types.map( fieldType => {
        ...
        switch(fieldType) {
            ...
            case 'date':
                row.push('date');
                console.log('ERROR PART:', row, field);
                if (field.min instanceof Date) row.push(field.min.toISOString()); else row.push('');
                if (field.max instanceof Date) row.push(field.max.toISOString()); else row.push('');
                row.push('');
                break;
        }
...
function getTopValues(uniques, count) {
    ...
    // TODO: If top result has count of 1, the field is a unique key
